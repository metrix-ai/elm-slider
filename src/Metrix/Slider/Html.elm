...
htmlStyleCss =
    node "style"
        ...
        [ "@import url(\"/css/document.css\");"
            ...
            ++ ".style-elements .__3402792823 {  border-style: solid;}"
            |> text
        ]
...
thumb left state =
    circle 12
        ...
        , ( "top", "12px" )
        , ( "cursor", "pointer" )
        , ( "background", colorToCssRgba state.colors.thumb )
        , ( "box-shadow", "-1px 0 2px 0 " ++ colorToCssRgba state.colors.outlinethumbBlur )
        ]
...
scale state =
    let
        color =
            Color.Interpolate.interpolate Color.Interpolate.RGB state.colors.inactiveBar state.colors.activeBar state.activeFactor |> colorToCssRgba
    in
    div
        [ style
            [ ( "width", toString state.scaleWidth ++ "px" )
            , ( "height", "6px" )
            , ( "margin", "-3px" )
            , ( "text-align", "center" )
            , ( "position", "relative" )
...
textLabel state indexDot str =
    let
        enter =
            ME.onMouseEnter (\_ -> Update.EnterLabel indexDot)

        leave =
            ME.onMouseLeave (\_ -> Update.LeaveLabel)

        default =
            ( colorToCssRgba state.colors.inactiveLabel, "400" )
...
over elements =
    let
        insert elem inner =
            div [ style [ ( "position", "absolute" ) ] ] [ inner, elem ]
    in
